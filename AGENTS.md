# Video Game Artist repository instructions

This repository contains one standalone C-Sweet protocol-v2 agent. Its purpose is:

> Creates and curates project assets under the approved asset-production strategy.

## Invariants

- Keep `com.csweet.video-game-artist` and version `1.0.0` synchronized between agent code,
...
```powershell
dotnet test
dotnet run --project src/CSweet.Agent.Artist.VideoGame -- --self-test
```
